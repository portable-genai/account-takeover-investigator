...
(``ATOINVEST_REVIEW_ROUTING=off``), the container binds :class:`DisabledReviewRouter` instead
...
investigation, but it is never invisible: the caller reports ``failed`` and an empty reference,
