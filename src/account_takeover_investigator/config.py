...
One env var (``ATOINVEST_PROFILE``) selects the adapter family for every
...
from .domain.policy import FusionPolicy
...
_PROFILE_ENV = "ATOINVEST_PROFILE"
_SETTINGS_ENV = "ATOINVEST_SETTINGS"
...
class ProfileChoice:
    ...
    The variable is ``ATOINVEST_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("ATOINVEST_PROFILE", "local")``: that fallback reads an
...
_PKG = "account_takeover_investigator"
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "sessions": {
        "local": f"{_PKG}.adapters.local.sessions:LocalSessionAdapter",
        "gcp": f"{_PKG}.adapters.gcp.sessions:CloudSessionAdapter",
        "onprem": f"{_PKG}.adapters.onprem.sessions:OnPremSessionAdapter",
    },
    "feature_store": {
        "local": f"{_PKG}.adapters.local.feature_store:LocalFeatureStoreAdapter",
        "gcp": f"{_PKG}.adapters.gcp.feature_store:CloudFeatureStoreAdapter",
        "onprem": f"{_PKG}.adapters.onprem.feature_store:OnPremFeatureStoreAdapter",
    },
    "iam_actions": {
        "local": f"{_PKG}.adapters.local.iam_actions:LocalIamActionsAdapter",
        "gcp": f"{_PKG}.adapters.gcp.iam_actions:CloudIamActionsAdapter",
        "onprem": f"{_PKG}.adapters.onprem.iam_actions:OnPremIamActionsAdapter",
    },
    "narrator": {
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``ATOINVEST_SETTINGS``) that does not
...
def _policy_block(data: Mapping[str, Any]) -> Mapping[str, Any] | None:
    """The settings ``policy:`` mapping, or ``None`` (defaults) when absent or empty."""
    block = data.get("policy")
    if block is None:
        return None
    if not isinstance(block, dict):
        raise ValueError("settings 'policy' must be a mapping of tunable -> value")
...
class Container:
    ...
    @cached_property
    def sessions(self) -> SessionSignalPort:
        adapter = self._bind("sessions")
        assert isinstance(adapter, SessionSignalPort)
        return adapter

    @cached_property
    def feature_store(self) -> FeatureStorePort:
        adapter = self._bind("feature_store")
        assert isinstance(adapter, FeatureStorePort)
        return adapter

    @cached_property
    def iam_actions(self) -> IamActionsPort:
        adapter = self._bind("iam_actions")
        assert isinstance(adapter, IamActionsPort)
        return adapter
