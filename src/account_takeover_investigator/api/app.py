"""FastAPI application for Account Takeover Investigator (G4).
...
``ATOINVEST_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``ATOINVEST_S2S_TOKEN`` is wrong in the most dangerous direction:
...
from .schemas import HealthResponse, InvestigateRequest, InvestigateResponse
...
_INSECURE_DEMO_ENV = "ATOINVEST_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "ATOINVEST_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "ATOINVEST_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "ATOINVEST_S2S_AUDIENCE"
_API_HOST_ENV = "ATOINVEST_API_HOST"
...
def _investigation_service(container: Container) -> InvestigationService:
    """Assemble the investigation service from the container's ports and the bank policy.

    The single place that knows which ports the service needs. Note the absence of
    ``iam_actions``: the investigation path never enacts a containment, so the service is not
    even given the port that could.
    """
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``ATOINVEST_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``ATOINVEST_PROFILE`` must never be allowed to
    ...
    ``ATOINVEST_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "ATOINVEST_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "account-takeover-investigator"
...
_APP_TITLE = "Account Takeover Investigator"
...
#
# Note what is NOT in this expression: ATOINVEST_S2S_TOKEN. A service
...
@app.post("/v1/investigate", response_model=InvestigateResponse, tags=["artifacts"])
def investigate(
    request: InvestigateRequest,
    principal: Annotated[Principal, Depends(get_principal)],
) -> InvestigateResponse:
    """Investigate a session; the actor AND the tenant are the verified principal, never the body.

    Both halves of the identity come from ``principal`` and from nowhere else. The tenant was
    read as ``request.tenant or principal.tenant``, which is not a fallback: any non-empty string
    the caller wrote WON over the tenant the identity adapter had verified, and the request still
    answered 200. The field is gone from ``InvestigateRequest`` as well, so the surface no longer
    ...
    return InvestigateResponse.from_domain(result, review_ref=review_ref)
