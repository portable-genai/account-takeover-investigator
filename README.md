# Account Takeover Investigator (G4)

Deterministic account-takeover signal fusion into cited, human-reviewed summaries.
...
account_takeover_investigator investigate acct-takeover sess-1
```
...
One env var, `ATOINVEST_PROFILE`, selects the adapter family:
...
`ATOINVEST_ALLOW_INSECURE_DEMO=1` says otherwise. The guard reads the identity
BINDING to decide that, never a service credential: setting
`ATOINVEST_S2S_TOKEN` closes the S2S routes and does not open anything else.
See `docs/runbook.md`.
...
| `pii-kit` | the jurisdiction PII pattern pack the investigation service redacts with |
...
**Name the profile.** `ATOINVEST_PROFILE` has no default. Leaving it unset is
