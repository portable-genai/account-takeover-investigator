# Runbook: Account Takeover Investigator (G4)

## Deploy (gcp)
1. `ATOINVEST_PROFILE=gcp`, install `.[gcp]`, region `asia-southeast1`.
...
3. Ingress is fronted by IAP/Apigee; the app authenticates the S2S caller fail-closed
   (`ATOINVEST_S2S_TOKEN` local, Google OIDC + allowlist secure).
4. **Set `ATOINVEST_IAP_AUDIENCE`.** Without it the service starts, stays
...
`ATOINVEST_IAP_AUDIENCE` is the IAP-protected resource the assertion must be
...
| Symptom | Cause | Fix |
|---|---|---|
| 503, detail names `ATOINVEST_IAP_AUDIENCE` | no audience configured | set it to the protected resource above |
...
| `gcp` IAP assertion | `verified` | authenticated: the signature (against IAP's own key set), the audience (against `ATOINVEST_IAP_AUDIENCE`), the expiry and the issuer are all checked before any claim is read |
...
1. **Nobody chose a profile.** `ATOINVEST_PROFILE` is absent, so no end-user
...
2. **The `local` profile, chosen deliberately.** The seeded personas are a client-asserted
   ...
   `ATOINVEST_S2S_TOKEN` is set. Setting that secret closes the S2S
   dependency and nothing else: it authenticates a calling SERVICE and authenticates no end user,
   so it cannot make `/v1/investigate` or `/v1/personas` authenticated and it does NOT switch the
   guard off. Were it to, a LAN peer with no credential at all would receive the full seeded
   persona list, approver included, and a real escalated investigation decision.
3. **The `onprem` profile with the placeholder still bound.** No identity provider is wired, so
   no end user can be authenticated. `/v1/investigate` answers 501 with the reason and the name of the
...
and neither carries per-caller data) while `/v1/investigate` answers 401 without an IAP assertion. The
...
- `add_loopback_exposure_guard` is registered at module scope in `api/app.py`, so it holds under
  `uvicorn account_takeover_investigator.api.app:app --host 0.0.0.0` (what the Dockerfile `CMD`
...
Set `ATOINVEST_ALLOW_INSECURE_DEMO=1` to accept the exposure deliberately.
...
`ATOINVEST_PROFILE` is read once, in `config.py`, and it has three states:

| State | What happens |
|---|---|
| unset | No choice was recorded. The SDK-free adapters bind (the alternative is importing cloud SDKs that are not installed), but every relaxation is withdrawn and the exposure guard refuses every route to any non-loopback peer. Symptom: 401 on `/v1/investigate` naming the variable, and 503 naming the `unconfigured` posture from off-box. Fix: set the variable. |
...
`ATOINVEST_S2S_TOKEN`. With routing on and the URL unset, the managed profile REFUSES TO BOOT,
...
`ATOINVEST_REVIEW_ROUTING` switches routing, read in three states: unset is on, `true`/`false`
...
`ATOINVEST_AUDIT_ANCHOR` (read by `audit_anchor_path` in
`config/settings.yaml`) to a file on a DIFFERENT volume from
`ATOINVEST_AUDIT_PATH`, ideally writable by a different principal. This is
not decoration:
