...
required CI check (the hosted GitHub Actions check) rather than inside `make gate`, because
...
Yes. the hosted GitHub Actions check is a thin caller of the shared reusable hard-gate workflow,
