...
from account_takeover_investigator.domain import (
    kernel,
    models,
)
from account_takeover_investigator.domain.fusion_engine import (
    FusionEngine,
)
from account_takeover_investigator.domain.investigation_service import (
    InvestigationService,
)
from account_takeover_investigator.domain.pii import (
    JURISDICTIONS,
)
...
SERVICE_NAME = "Account Takeover Investigator"
CATALOG_ID = "G4"
REPOSITORY = "account-takeover-investigator"
...
#: than trusting the pattern pack to agree with itself. It rides on the subject id below.
PLANTED_NRIC = "S1234567D"

#: A quiet account on a known device: LOW, nothing routed.
ROUTINE_CASE = models.InvestigationRequest(subject_id="acct-quiet", session_id="sess-1")

#: A full takeover (new device, impossible travel, biometric drift): CRITICAL, routed.
ESCALATING_CASE = models.InvestigationRequest(subject_id="acct-takeover", session_id="sess-1")

#: An escalating case whose subject id carries a planted national id, for the redaction beat.
PII_CASE = models.InvestigationRequest(
    subject_id="acct-takeover-" + PLANTED_NRIC, session_id="sess-1"
)
...
class DemoRun:
    ...
    def _step_routine(self) -> Produced:
        return self._investigate_panels(ROUTINE_CASE, expect_routing=False)

    def _step_escalation(self) -> Produced:
        return self._investigate_panels(ESCALATING_CASE, expect_routing=True)

    def _step_redaction(self) -> Produced:
        panels, facts = self._investigate_panels(PII_CASE, expect_routing=True)
    ...
    def _investigate_panels(
        self, case: models.InvestigationRequest, *, expect_routing: bool
    ) -> Produced:
        result = self.service.investigate(case, actor=ACTOR)
        self.cases += 1
        review_ref = ""
        ...
        decision = Panel(
            title="Assessment: " + result.subject,
            rows=(
                Row(
                    "Risk band", result.band.value, "bad" if result.requires_human_review else "ok"
                ),
                Row("Fused score", f"{result.score:.2f}"),
                Row("Decision", result.decision.value),
                Row("Requires human review", str(result.requires_human_review)),
                Row(
                    "Routed to review",
                    review_ref or "not routed (low risk)",
                    "ok" if consistent else "bad",
                ),
                Row("Attributed to", ACTOR),
            ),
            note=(
                "The score and band are computed by pure stdlib code and are replayable. A model "
                "narrates this result; it never produces the number."
            ),
            tone="ok" if consistent else "bad",
        )
        uplift_rows = [
            Row(s.kind.value, f"+{s.uplift:.2f}  {s.summary}") for s in result.signals
        ] or [Row("signals", "none fired; baseline only", "ok")]
        uplift_rows.append(Row("fused score", f"= {result.score:.2f} ({result.band.value})"))
        fusion = Panel(
            title="Per-signal uplift arithmetic",
            rows=tuple(uplift_rows),
            note=(
                "Baseline plus one uplift line per fired signal, clamped. The reviewer can "
                "recompute the score by hand from these rows."
            ),
        )
        containment = Panel(
            title="Recommended containment (never enacted here)",
            rows=tuple(
                Row(c.action.value, c.rationale, "bad" if c.requires_human_review else "ok")
                for c in result.containments
            ),
            note=(
                "Containment is recommended, then routed for human approval. This service never "
                "calls the IAM action port on the investigation path."
            ),
        )
        facts = {
        ...
        return [decision, fusion, containment], facts
...
def _exit_review(container: Any) -> Any:
    citation = kernel.Citation(source_id="session:sess-1", title="Session", snippet="takeover")
    return container.review_router.route(
        models.Investigation(
            subject="acct-takeover",
            session_id="sess-1",
            tenant=TENANT,
            severity=kernel.Severity.CRITICAL,
            decision=kernel.Decision.ESCALATED,
            band=models.RiskBand.CRITICAL,
            score=1.0,
            baseline_score=0.0,
            summary="acct-takeover: CRITICAL account-takeover risk (1.00)",
            narrative="synthetic",
            signals=(),
            containments=(),
...
def _exit_sessions(container: Any) -> Any:
    return container.sessions.fetch("acct-takeover", "sess-1")


def _exit_feature_store(container: Any) -> Any:
    return container.feature_store.fetch("acct-takeover")


def _exit_narrator(container: Any) -> Any:
    from account_takeover_investigator.domain.narrative import NarrativeBrief
...
def _exit_iam_actions(container: Any) -> Any:
    rec = models.ContainmentRecommendation(
        action=models.ContainmentAction.LOCK_SESSION,
        severity=kernel.Severity.CRITICAL,
        rationale="synthetic",
        requires_human_review=True,
    )
    return container.iam_actions.enact(rec, subject_id="acct-takeover", actor=ACTOR)
