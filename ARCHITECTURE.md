# ARCHITECTURE: Account Takeover Investigator (G4)

Hexagonal ports-and-adapters. A pure-stdlib domain core speaks only to ports (`typing.Protocol`s);
adapter families implement them; one env var (`ATOINVEST_PROFILE`) swaps the
...
## Layout (`src/account_takeover_investigator/`)
- `domain/` : pure stdlib, no cloud/framework imports. `kernel.py` (vertical-neutral types,
  `StrEnum` taxonomies from the commons), `models.py` (the account-takeover artifacts: raw intake
  and derived signals), `policy.py` (the bank-owned thresholds and weights), `fusion_engine.py`
  (the deterministic signal-fusion engine), `narrative.py` (the grounded narration brief and the
