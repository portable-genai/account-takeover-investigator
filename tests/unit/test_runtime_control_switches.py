...
control this service has: ``ATOINVEST_REVIEW_ROUTING`` is read in three states; off binds a
...
an already-scored investigation when the console is unreachable.
"""
...
from account_takeover_investigator.adapters.controls import (
...
from account_takeover_investigator.domain.models import Investigation
from account_takeover_investigator.envread import ConfiguredEmptyError

from tests.fixtures import sample_cases

_LOOPBACK = ("127.0.0.1", 50000)
_ESCALATING = sample_cases.ESCALATING_REQUEST
_ROUTINE = sample_cases.ROUTINE_REQUEST
...
def _managed(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "account_takeover_investigator.config.resolve_profile",
...
def test_the_off_posture_is_logged_once_however_many_containers(
    ...
    settings = Settings(profile="local", controls=ControlSwitches(review_routing=False))
    with caplog.at_level(logging.WARNING, logger="account_takeover_investigator.config"):
        for _ in range(3):
...
# --------------------------------------------------------------------------- #
class _Accepting:
    def route(self, result: Investigation, *, maker: str, tenant: str = "") -> str:
        return "review-1"
...
def test_routing_outcomes_take_each_of_their_four_values() -> None:
    escalated = sample_cases.make_investigation()
    quiet = sample_cases.make_investigation(requires_human_review=False)
...
def test_a_failed_hand_off_is_reported_and_logged_never_raised(
    ...
    failed = RecordingReviewRouter(_Refusing())
    with caplog.at_level(logging.WARNING, logger="account_takeover_investigator.adapters.controls"):
        assert failed.route(sample_cases.make_investigation(), maker="m") == ""
    assert failed.outcome is ReviewRouting.FAILED
...
# --------------------------------------------------------------------------- #
def _investigate(subject_id: str = _ESCALATING.subject_id) -> dict[str, object]:
    response = TestClient(app, client=_LOOPBACK).post(
        "/v1/investigate",
        json={"subject_id": subject_id, "session_id": _ESCALATING.session_id},
...
def test_the_api_reports_nothing_to_route() -> None:
    body = _investigate(_ROUTINE.subject_id)
...
def test_the_api_reports_a_failed_hand_off_instead_of_failing_the_investigation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        "account_takeover_investigator.adapters.local.review_router.LocalReviewRouter.route",
        _Refusing.route,
    )
    body = _investigate()
...
def test_the_agent_tool_reports_the_hand_off() -> None:
    payload = tools.investigate_session(
        _ESCALATING.subject_id, _ESCALATING.session_id, tenant=sample_cases.TENANT
    )
...
def test_the_cli_reports_the_hand_off(
    ...
    monkeypatch.setenv(REVIEW_ROUTING_ENV, "off")
    assert cli.main(["investigate", _ESCALATING.subject_id, _ESCALATING.session_id]) == 0
