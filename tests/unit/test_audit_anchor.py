...
from account_takeover_investigator.adapters.local.audit import (
    LocalAuditAdapter,
)
...
_ANCHOR_ENV = "ATOINVEST_AUDIT_ANCHOR"
...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    for subject in ("acct-takeover", "acct-quiet", "acct-stuffing"):
        service.investigate(
            InvestigationRequest(
                subject_id=subject, session_id="sess-1", tenant=sample_cases.TENANT
            ),
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    with pytest.raises(AuditChainError):
        _service(build_container(settings)).investigate(
            InvestigationRequest(subject_id="acct-takeover", session_id="s", tenant=""),
            actor=sample_cases.ACTOR,
        )
