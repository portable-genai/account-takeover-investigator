...
def test_the_demo_data_is_obviously_fictional() -> None:
    ...
    from account_takeover_investigator.adapters.local import fixtures

    cases = (demo.ROUTINE_CASE, demo.ESCALATING_CASE, demo.PII_CASE)
    for case in cases:
        assert case.subject_id.startswith("acct-"), case.subject_id + " is not a synthetic account"
    assert demo.PLANTED_NRIC in demo.PII_CASE.subject_id, "the PII beat needs a planted identifier"
    blob = demo.ACTOR + demo.TENANT
    ...
    # The offline session fixtures use RFC 5737 documentation literals, never a routable address.
    source = inspect.getsource(fixtures)
    assert "192.0.2." in source and "198.51.100." in source
