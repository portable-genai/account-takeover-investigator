...
from account_takeover_investigator.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from account_takeover_investigator.config import Settings
from account_takeover_investigator.ports.identity import (
    VERIFIED,
...
_API_MODULE = "account_takeover_investigator.api.app"
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        "  sessions:",
        *[f"    {p}: {_PKG}.adapters.local.sessions:LocalSessionAdapter" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.sessions:OnPremSessionAdapter",
        "  feature_store:",
        *[
            f"    {p}: {_PKG}.adapters.local.feature_store:LocalFeatureStoreAdapter"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.feature_store:OnPremFeatureStoreAdapter",
        "  iam_actions:",
        *[
            f"    {p}: {_PKG}.adapters.local.iam_actions:LocalIamActionsAdapter"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.iam_actions:OnPremIamActionsAdapter",
        "  narrator:",
        *[
            f"    {p}: {_PKG}.adapters.local.narrator:LocalNarratorAdapter"
            for p in ("local", "gcp")
        ],
...
def test_a_malformed_assertion_from_a_lan_peer_answers_401_and_never_500(
    ...
    with TestClient(module.app, client=(LAN_PEER, 50000)) as client:
        resp = client.post(
            "/v1/investigate",
            json={"subject_id": "acct-takeover", "session_id": "sess-1"},
