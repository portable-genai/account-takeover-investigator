...
from account_takeover_investigator.adapters._review_payload import (
    result_to_review,
)
...
def _investigation_service(container: object) -> InvestigationService:
    return InvestigationService(
        container.sessions,  # type: ignore[attr-defined]
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    result = _investigation_service(local).investigate(
    ...
    with pytest.raises(NotImplementedError):
        _investigation_service(onprem).investigate(
            sample_cases.ESCALATING_REQUEST, actor=sample_cases.ACTOR
        )
