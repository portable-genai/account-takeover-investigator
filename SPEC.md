# SPEC: Account Takeover Investigator (G4)
...
- **Redaction before audit**: the investigation service redacts PII (via `pii-kit`) before writing any
  audit record. No raw identifier reaches the WORM store.
- **Domain**: the sessions and feature-store ports return raw cited observations (the current
  session, its device and geography, the recent login history, the account baseline); the pure
  `FusionEngine` fuses four signals (device change, impossible travel, credential-stuffing
  velocity, behavioural-biometric deviation) into a score, a band and a containment set, one
  auditable uplift line per signal. Containment is RECOMMENDED, never enacted: the service does
  not depend on `IamActionsPort`, and a consequential recommendation is routed for approval.
- **Determinism**: the fused score, the band and the containment recommendation are pure stdlib
  and replayable (byte-identical for the same inputs and policy); an LLM may narrate but never
  produces a number or a verdict, and its narration is discarded for a deterministic draft if it
  states a figure the engine did not.
- **Maker-checker (P-06) and routing (R8)**: a HIGH/CRITICAL result sets
  ...
  with routing on and no console configured; `ATOINVEST_REVIEW_ROUTING=off` is the stated way to
...
- **Profile**: resolved ONCE, at import, into a `ProfileChoice` and never a bare string. Three
  states of `ATOINVEST_PROFILE`: UNSET is NO CHOICE (the SDK-free adapters
