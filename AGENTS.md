...
Account Takeover Investigator (G4): Deterministic account-takeover signal fusion into cited, human-reviewed summaries.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `account_takeover_investigator`, environment prefix
`ATOINVEST`, region `asia-southeast1`.
...
- `src/account_takeover_investigator/domain/` is PURE stdlib. No web framework, no cloud SDK,
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `ATOINVEST_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `ATOINVEST_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `ATOINVEST_PROFILE` (mentioning it in a refusal message is wanted, reading
